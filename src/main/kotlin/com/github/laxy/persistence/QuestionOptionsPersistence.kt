...
fun questionOptionsPersistence(questionOptionsQueries: QuestionOptionsQueries) = object : QuestionOptionsPersistence {
    ...
    override suspend fun insertQuestionOption(
        ...
        either {
            val questionOptionId = questionOptionsQueries
                .insertAndGetId(questionId, description, referenceNumber, isCorrect)
                .executeAsOneOrNull()
            ensureNotNull(questionOptionId) { QuestionOptionCreationError("questionOptionId=$questionOptionId") }
        }
    }
}
