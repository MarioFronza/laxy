...
interface QuizPersistence {
    ...
    suspend fun insertQuiz(userId: UserId, subjectId: SubjectId, totalQuestions: Int): Either<DomainError, QuizId>
    suspend fun updateStatus(quizId: QuizId, status: String)
    suspend fun deleteQuiz(quizId: QuizId)
}

fun quizPersistence(quizzesQueries: QuizzesQueries) = object : QuizPersistence {
    ...
    override suspend fun selectById(quizId: QuizId) = withSpan("$spanPrefix.selectById") {
        either {
    ...
    override suspend fun insertQuiz(userId: UserId, subjectId: SubjectId, totalQuestions: Int) =
        withSpan("$spanPrefix.insertQuiz") {
            either {
                val quizId = quizzesQueries.insertAndGetId(userId, subjectId, totalQuestions).executeAsOneOrNull()
                ensureNotNull(quizId) { QuizCreationError("quizId=$quizId") }
            }
        }

    override suspend fun updateStatus(quizId: QuizId, status: String) = withSpan("$spanPrefix.updateStatus") {
        quizzesQueries.updateStatus(status, quizId)
    }

    override suspend fun deleteQuiz(quizId: QuizId) = withSpan("$spanPrefix.deleteQuiz") {
