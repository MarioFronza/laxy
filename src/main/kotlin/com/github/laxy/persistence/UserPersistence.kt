...
interface UserPersistence {
    ...
    suspend fun update(
        userId: UserId, email: String?, username: String?, password: String?
...
fun userPersistence(
    ...
    override suspend fun verifyPassword(
        ...
        val (userId, username, salt, key) = ensureNotNull(
    ...
    override suspend fun select(userId: UserId): Either<DomainError, UserInfo> = either {
        val userInfo =
            usersQueries.selectById(userId) { email, username, _, _ -> UserInfo(username, email) }.executeAsOneOrNull()
    ...
    override suspend fun update(
        ...
        val info = usersQueries.transactionWithResult {
            usersQueries.selectById(userId).executeAsOneOrNull()?.let { (oldEmail, oldUsername, salt, oldPassword) ->
...
private fun UsersQueries.create(
    username: String, email: String, salt: ByteArray, key: ByteArray
): UserId = insertAndGetId(username = username, email = email, salt = salt, hashed_password = key).executeAsOne()
