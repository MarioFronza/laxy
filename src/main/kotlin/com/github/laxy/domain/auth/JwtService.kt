...
fun jwtService(env: Env.Auth, persistence: UserPersistence) =
    object : JwtService {
        ...
        override suspend fun verifyJwtToken(token: JwtToken): Either<DomainError, UserId> = either {
            val jwt = JWT.decodeT(token.value, JWSES512Algorithm).mapLeft { JwtInvalid(it.toString()) }.bind()
            val userId = ensureNotNull(jwt.claimValueAsLong("id").getOrNull()) {
                JwtInvalid("id missing from JWT Token")
            }
            val expiresAt = ensureNotNull(jwt.expiresAt().getOrNull()) { JwtInvalid("exp missing from JWT Token") }
