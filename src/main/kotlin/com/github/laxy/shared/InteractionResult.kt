...
data class Failure<E, D>(val error: E) : InteractionResult<E, D>() {
    ...
    override fun <ME, MD> map(failure: (E) -> ME, success: (D) -> MD) = Failure<ME, MD>(failure(error))
    override fun <ME> mapFailure(transform: (E) -> ME): InteractionResult<ME, D> = Failure(transform(error))
...
data class Success<E, D>(private val data: D) : InteractionResult<E, D>() {
    ...
    override fun <ME, MD> map(failure: (E) -> ME, success: (D) -> MD) = Success<ME, MD>(success(data))
    override fun <ME> mapFailure(transform: (E) -> ME): InteractionResult<ME, D> = Success(data)
    override fun <MD> mapSuccess(transform: (D) -> MD): InteractionResult<E, MD> = Success(transform(data))
}
