...
data class UpdateUser(
    val email: String? = null, val username: String? = null, val password: String? = null
)

@Serializable
data class User(
    val email: String, val token: String, val username: String
)
...
fun Route.userRoutes(
    ...
    put<UserResource> {
        jwtAuth(jwtService) { (token, userId) ->
            either {
                val (email, username, password) = receiveCatching<UserWrapper<UpdateUser>>().bind().user
