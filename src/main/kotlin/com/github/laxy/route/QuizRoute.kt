...
@Resource("/quizzes") data class QuizzesResource(val parent: RootResource = RootResource)
...
fun Route.quizRoutes(quizService: QuizService) {
    post<QuizzesResource> {
        either {
                ...
                val quiz = quizService.createQuiz(CreateQuiz(SubjectId(subjectId), totalQuestions))
                QuizWrapper(quiz)
            }
