...
import com.github.laxy.shared.InvalidEmail
import com.github.laxy.shared.InvalidPassword
import com.github.laxy.shared.InvalidUsername
...
class UserServiceSpec : SuspendFun({
    ...
    "register" - {
        "given an empty username, should return IncorrectInput when calls register" {
            val res = userService.register(
                RegisterUser(username = "", email = validEmail, password = validPassword)
            )
        ...
        "given a too long username, should return IncorrectInput when calls register" {
            ...
            val res = userService.register(
                RegisterUser(username = name, email = validEmail, password = validPassword)
            )
        ...
        "given an empty email, should return IncorrectInput when calls register" {
            val res = userService.register(
                RegisterUser(username = validUsername, email = "", password = validPassword)
            )
        ...
        "given a too long email, should return IncorrectInput when calls register" {
            ...
            val res = userService.register(
                RegisterUser(username = validUsername, email = email, password = validPassword)
            )
        ...
        "given an empty password, should return IncorrectInput when calls register" {
            val res = userService.register(
                RegisterUser(username = validUsername, email = validEmail, password = "")
            )
        ...
        "given a too long password, should return IncorrectInput when calls register" {
            ...
            val res = userService.register(
                RegisterUser(username = validUsername, email = validEmail, password = password)
            )
        ...
        "given a valid input, should return Success when calls register" {
            userService.register(
                RegisterUser(username = validUsername, email = validEmail, password = validPassword)
            ).shouldBeRight()
        }
    ...
    "login" - {
        "given an empty email, should return IncorrectInput when calls login" {
            val res = userService.login(
                Login(email = "", password = validPassword)
            )
            val errors = nonEmptyListOf("Cannot be blank", "'' is invalid email")
            val expected = IncorrectInput(InvalidEmail(errors))
            res shouldBeLeft expected
        ...
        "given a too long email, should return IncorrectInput when calls login" {
            val email = "${(0..340).joinToString("") { "A" }}@domain.com"
            val res = userService.login(
                Login(email = email, password = validPassword)
            )
            val errors = nonEmptyListOf("is too long (maximum is 350 characters)")
            val expected = IncorrectInput(InvalidEmail(errors))
            res shouldBeLeft expected
        ...
        "given an empty password, should return IncorrectInput when calls login" {
            val res = userService.login(
                Login(email = validEmail, password = "")
            )
            val errors = nonEmptyListOf("Cannot be blank", "is too short (minimum is 8 characters)")
            val expected = IncorrectInput(InvalidPassword(errors))
            res shouldBeLeft expected
    ...
    "update" - {
        "given a valid input, should update with all null values when calls update" {
            val token = userService.register(RegisterUser(validUsername, validEmail, validPassword)).shouldBeRight()
            val res = userService.update(UpdateUser(token.id(), null, null, null))
